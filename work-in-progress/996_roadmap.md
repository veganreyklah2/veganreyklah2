...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; sixty-four witnesses, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each witness `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 64 programs and RED on tamper (`parity-selftest.rish`).
...
- **Rishi string builtins** — `length`, `trim`, `slice`, `lines`, `starts-with`, `ends-with`, `split`, `join`, `contains`, infix `index-of`; gate trio 64/64 GREEN.
- **Skate text grid** — monospace 8×8 glyphs on Brushstroke Wayland seed; headless selftest green (`10023` Track A).
- **Strengthening passes 9979–9939** — `trimStart` … `mem.startsWith`. Each through the gate; witness count 17 → 64.
...
- [x] **Strengthening 9979–9939** — everyday `std` surfaces through the gate (`trimStart` … `mem.startsWith`; see `strengthening-compiler/9939`–`9987`).
...
1. **Strengthening series** — next `std` surface through the gate trio (`9938` and below; `998`)
...
**Aurora metal lane** (`200312`, detail in `995`): the freestanding path already rides strengthened **crypto** and **`mem.eql`**; RISC-V uses the **scalar** `eql` path. Next std surfaces that matter on metal after the current mem arc: **`endsWith`** (9938), then **`crypto.timing_safe`** and Keccak **`@memcpy`** slice bounds. A freestanding **integration smoke** (`aurora/run.sh` in CI) complements — it does not replace — hosted `parity.rish`.
...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, **SemanticVersion** trio (**format 9946**, **parse 9947**, **order 9948**), **mem.sliceTo (9945)**, **copyBackwards (9944)**, **copyForwards (9943)**, **mem.len (9942)**, **mem.eql (9941)**, **mem.allEqual (9940)**, **mem.startsWith (9939)**, mem compare/count through **findDiff (9949)** onward, mem iterators through **WindowIterator (9954)**, mem.join (9961), cut family (9958–9960) — all done. **Strengthening the next `std` surface (9938)** stays the honest next step on the main track; **RyeVersion** accretion waits beside the capability table.

---

## The Steps Just Taken

Strengthening through **9939**, garden-memory policy in TAME and rules, Rishi builtins and parser fix, Skate grid, parity via `rye run`, witnesses vocabulary, style audit shipped.

**Strengthening passes 9979–9939** (`143312`–`200012`): `trimStart` … `mem.startsWith`. Witness count 17 → 64, all green.
