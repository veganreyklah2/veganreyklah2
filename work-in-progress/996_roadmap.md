...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, **TAME document stack (`014512`)**, **SemanticVersion** trio, **mem** compare/copy/sentinel arc, **timing_safe.eql (9937)** on the metal path, **Keccak/SHAKE `@memcpy` (9936 + 9935)** on the metal path, mem iterators through **WindowIterator (9954)**, mem.join (9961), cut family (9958–9960), **split factories (9934 + 9933)**, **tokenize factories (9932)**, **window factory (9931)**, **mem.trim (9930)**, **crypto.secureZero (9929)**, **mem.span (9928)**, **Tally explicit widths (`211712`)**, **`caravan/seed` widths (`014512`)** — done. **Strengthening (`9927`)** via `k`; **width Phase 1** continues `caravan/bounded` → Skate between `k` runs. **Freestanding smoke** on Aurora metal lane; **RyeVersion** beside capability table.
