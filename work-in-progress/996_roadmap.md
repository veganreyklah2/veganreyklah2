...
**Language:** EN
**Version:** `20260620.165112` (Rye chronological stamp)
...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; the corpus carries thirty-four programs, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each corpus `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 34 programs and RED on tamper (`parity-selftest.rish`).
...
- **Rishi string builtins** — `length`, `trim`, `slice`, `lines`, `starts-with`, `ends-with`, `split`, `join`, `contains`, infix `index-of`; gate trio 34/34 GREEN.
- **Skate text grid** — monospace 8×8 glyphs on Brushstroke Wayland seed; headless selftest green (`10023` Track A).
- **Strengthening passes 9979–9969** — `trimStart` … `SplitIterator.peek`. Each through the gate; corpus 17 → 34.
...
- [x] **Strengthening 9979–9969** — everyday `std` surfaces through the gate (`trimStart` … `SplitIterator.peek`; see `strengthening-compiler/9969`–`9987`).
...
1. **Strengthening series** — next `std` surface through the gate trio (`9968` and below; `998`)
...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, mem search, and **SplitIterator.peek (9969)** — all done. **Strengthening the next `std` surface (9968)** is the honest next step on the main track before virtio-net or the capability table widen scope.

---

## The Steps Just Taken

Strengthening through **9969**, garden-memory policy in TAME and rules, Rishi builtins and parser fix, Skate grid, parity via `rye run`, style audit shipped.

**Strengthening passes 9979–9969** (`143312`–`165112`): `trimStart` … `SplitIterator.peek`. Corpus 17 → 34, all green.
