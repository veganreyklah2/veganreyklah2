...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; sixty-two witnesses, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each witness `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 62 programs and RED on tamper (`parity-selftest.rish`).
...
- **Rishi string builtins** — `length`, `trim`, `slice`, `lines`, `starts-with`, `ends-with`, `split`, `join`, `contains`, infix `index-of`; gate trio 62/62 GREEN.
- **Skate text grid** — monospace 8×8 glyphs on Brushstroke Wayland seed; headless selftest green (`10023` Track A).
- **Strengthening passes 9979–9941** — `trimStart` … `mem.eql`. Each through the gate; witness count 17 → 62.
...
- [x] **Strengthening 9979–9941** — everyday `std` surfaces through the gate (`trimStart` … `mem.eql`; see `strengthening-compiler/9941`–`9987`).
...
1. **Strengthening series** — next `std` surface through the gate trio (`9940` and below; `998`)
...
Tally v1 gardens, Brix minimum, Mantra repo chain, Rishi gate trio, Caravan chain, Brushstroke Wayland, Comlink hosted wire, Skate glyph grid, path naming family, garden-memory policy, **SemanticVersion** trio (**format 9946**, **parse 9947**, **order 9948**), **mem.sliceTo (9945)**, **copyBackwards (9944)**, **copyForwards (9943)**, **mem.len (9942)**, **mem.eql (9941)**, **mem.allEqual (9940)**, mem compare/count through **findDiff (9949)** onward, mem iterators through **WindowIterator (9954)**, mem.join (9961), cut family (9958–9960) — all done. **Strengthening the next `std` surface (9939)** stays the honest next step on the main track; **RyeVersion** accretion waits beside the capability table.

---

## The Steps Just Taken

Strengthening through **9941**, garden-memory policy in TAME and rules, Rishi builtins and parser fix, Skate grid, parity via `rye run`, witnesses vocabulary, style audit shipped.

**Strengthening passes 9979–9941** (`143312`–`195412`): `trimStart` … `mem.eql`. Witness count 17 → 62, all green.
