...
- **The gate trio runs** — `parity.rish` (behavior), `parity-selftest.rish` (proves RED on divergence), `additive-gate.rish` (shape). All three run in Rishi; sixty-three witnesses, all green.
...
- **The parity gate runs in Rishi** — `tools/parity.rish` runs each witness `.rye` through `rye run` (baseline via `RYE_LIB=vendor/zig-toolchain/lib`, strengthened via default `rye/lib`), compares outputs as one value, and asserts the verdict. Proven GREEN across 63 programs and RED on tamper (`parity-selftest.rish`).
...
- **Rishi string builtins** — `length`, `trim`, `slice`, `lines`, `starts-with`, `ends-with`, `split`, `join`, `contains`, infix `index-of`; gate trio 63/63 GREEN.
- **Skate text grid** — monospace 8×8 glyphs on Brushstroke Wayland seed; headless selftest green (`10023` Track A).
- **Strengthening passes 9979–9940** — `trimStart` … `mem.allEqual`. Each through the gate; witness count 17 → 63.
...
- [x] **Strengthening 9979–9940** — everyday `std` surfaces through the gate (`trimStart` … `mem.allEqual`; see `strengthening-compiler/9940`–`9987`).
...
1. **Strengthening series** — next `std` surface through the gate trio (`9939` and below; `998`)
...
Strengthening through **9940**, garden-memory policy in TAME and rules, Rishi builtins and parser fix, Skate grid, parity via `rye run`, witnesses vocabulary, style audit shipped.

**Strengthening passes 9979–9940** (`143312`–`195612`): `trimStart` … `mem.allEqual`. Witness count 17 → 63, all green.
