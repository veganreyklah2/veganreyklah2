...
**Claude `214200` latch affirm:** seal **rests** — zero rewrites. Ceremony command on exit-card affirmation path (primary `!`). Doorway Status on `213600` note → **Checkable**. Crypto teacher **Monocypher** — must use `monocypher-ed25519` / `crypto_ed25519_check` (RFC 8032/SHA-512); default BLAKE2b EdDSA is the named red-to-avoid. Vendor `vendor/monocypher`. Libsodium parks (second guest crypto · or Sculpt-and-Goa). Living-docs lint thrice earned. Counsel [`../active-designing/20260712-214200_proven-seat-signed-kumara-monocypher.md`](../active-designing/20260712-214200_proven-seat-signed-kumara-monocypher.md).
