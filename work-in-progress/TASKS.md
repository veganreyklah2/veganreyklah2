...
- [x] **Wire chain-read seams** — `tools/chain_read_sui.rish` / `tools/chain_read_monad.rish`; `tools/ensure_grpcurl.sh` bootstraps gitignored `tools/.build/grpcurl` (not in `parity.rish` — network seam)
