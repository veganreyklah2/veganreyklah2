...
**Language:** EN
**Last updated:** 2026-07-11 (Cursor-bot arc audit `053412` — money code verified sound, three WOV money functions split+witnessed, one WOV/TigerBeetle self-contained tension flagged for Kaeden)
...
- [ ] **Open question for Kaeden — WOV's pinned-TigerBeetle vs the self-contained direction** — `tools/wov_tb_ensure_binary.sh` builds TigerBeetle from vendored/pinned `gratitude/tigerbeetle` source using a separate Zig 0.14.1 toolchain and runs it as an external process (clean-room, "never links TB into Rye"). Faithful to WOV's original design, yet in real tension with the no-external-interpreters direction set `041412`/`152612` — TB is a heavier external dependency than `grpcurl` was. Not ruled on (Kaeden's call): does WOV's pinned-TB foundation still fit self-contained, or does WOV want rethinking the way the settlement rail did? (`wov_tb_pin.rish` witness fails on this bench precisely because the external TB binary isn't present — reinforces the point.)
